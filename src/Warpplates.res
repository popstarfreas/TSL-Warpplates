...
module Map = TerrariaServerLite.Map
...
@bs.module("fs") external writeFile: (string, string, (Js.nullable<Js.Exn.t>) => unit) => unit = "writeFile"
let saveWarpplates = (self: t) => {
    let json = Js.Json.stringifyAny(self.warpplates)
    switch json {
        | Some(warpplates) => writeFile("./persistence/warpplates.json", warpplates, err => {
            switch err->Js.Nullable.toOption {
                | Some(err) => self.logger->Log.error("Could not write warpplates to file. " ++ err->Js.Exn.message->Belt.Option.getWithDefault("(Error message not found)"))
                | None => ()
            }
        })
...
let sendClientToDimension = (client: Client.t, dimension: string) => {
    ...
    client->Client.sendPacket(TerrariaServerLite.PacketWriter.make()
...
let packetHandler = TerrariaServerLite.ExtensionPacketHandler.make(
    (self: t, _extension: Extension.t, client: Client.t, packet: Packet.t) => {
                ...
                switch matchedWarpplate {
                    | Some({ dimension, noGuests }) => {
                        switch (client->Client.getUserAccount, noGuests) {
                            ...
                            | _ => {
                                let lastSentMessage = client->Client.extProperties->Map.get(messageTimeKey)
                                switch lastSentMessage {
                                    | Some(lastSentMessage) => {
                                        ...
                                        if Js.Date.now() -. lastSentMessage > 3000.0 {
                                            client->Client.sendChatMessage(~message="You need to be logged-in to use this warpplate.", ~color={
                                                \"R": 255,
                                                \"G": 0,
                                                \"B": 0,
                                            }, ())
                                            client->Client.extProperties->Map.set(messageTimeKey, Obj.magic(Js.Date.now()))
                                        }
                                    }
                                    | None => {
                                        client->Client.extProperties->Map.set(messageTimeKey, Obj.magic(Js.Date.now()))
